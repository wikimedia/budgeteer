...
class Budgeteer {
    ...
    check(key, limitConf, eventTimestamp = Date.now(), cost = 0) {
        ...
        .then((info) => {
            if (info.last_success > new Date(eventTimestamp)) {
    ...
    reportSuccess(key, limitConf, startTime, cost = 1) {
        ...
        return this._getInfo(key, limitConf)
        .then((info) => {
            info.last_success = new Date(startTime);
    ...
    _getInfo(key, limitConf) {
        ...
        .then((redisInfo) => {
            if (redisInfo) {
                ...
                try {
                    const info = JSON.parse(redisInfo);
                    return {
                        last_success: new Date(info[0]),
                        is_scheduled: !!info[1],
                        token_balance: info[2]
                    };
                } catch (e) {}
            }
            return {
    ...
    _putInfo(key, info) {
        ...
        const blob = JSON.stringify([
            Number(info.last_success),
            info.is_scheduled ? 1 : 0,
            info.token_balance
        ]);
        return this._store.put(key, blob);
    }
